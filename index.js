...
async function getAppwriteUserDoc(discordUsername) {
  ...
  const registeredUser = await db.listDocuments("db", "discordUsers", [
...
async function handleInteraction(interaction) {
  ...
  try {
    ...
    if (commandName === "like") {
      ...
      const myUser = user;
      const theirUser =
        (await client.users.fetch(theirAppwriteDoc.discordUserId)) || null;

      const doIAlreadyLikeThem = await db.listDocuments("db", "likes", [
        Query.equal("userA", [myUsername]),
        Query.equal("userB", theirUsername),
      ]);
      const doTheyAlreadyLikeMe = await db.listDocuments("db", "likes", [
        Query.equal("userA", [theirUsername]),
        Query.equal("userB", myUsername),
      ]);
      ...
      try {
        if (doTheyAlreadyLikeMe.total > 0) {
          const deleteLike = await db.deleteDocument(
            "db",
            "likes",
            doTheyAlreadyLikeMe.documents[0].$id,
          );
          if (theirUser) {
          ...
          const createLike = await db.createDocument(
            "db",
            "likes",
            ID.unique(),
            { userA: myUsername, userB: theirUsername },
            [Permission.read(Role.user(myAppwriteDoc.$id))],
          );
          const theirSubscription = await db.listDocuments(
            "db",
            "subscriptions",
            [
              Query.equal("username", [theirUsername]),
              Query.limit(1),
          ...
          if (theirSubscription.total > 0) {
            ...
            const subscriptionDate = new Date(
              theirSubscription.documents[0].endTimestamp,
            );
            const currentDate = new Date();
            if (subscriptionDate > currentDate) {
              await theirUser.send(`${myUsername} liked you`);
            } else {
              await theirUser.send(
                `Someone liked you. Subscribe to find out who it is`,
              );
            }
          } else {
            await theirUser.send(
      ...
      const mySubscription = await db.listDocuments("db", "subscriptions", [
        Query.equal("username", [myUsername]),
        Query.limit(1),
        Query.orderDesc("$createdAt"),
      ]);

      const getPreviewSubscription = await db.listDocuments(
        "db",
        "previews",
        [
          Query.equal("username", [myUsername]),
      ...
      if (mySubscription.total > 0) 
      {
          ...
          if(timestamp > theirSubscription.documents[0].endTimestamp) {
             if(getPreviewSubscription.total > 0) {
                ...
                message = `Your most recent subscription started <t:${subscriptionStartTimestamp.toString()}:R> and ended <t:${subscriptionEndTimestamp.toString()}:R> Click (here) to view your most recent admirers for free (one-time use)`;
              }
          } else {
      ...
      const index = parseInt(interaction.options.getString("duration"), 10);
      ...
      try {
        ...
        message = `[Click here to subscribe for ${lineItems[index].price_data.product_data.name}](${session.url})`;
      } catch (err) {
        message = "An error occurred";
        throw new Error(message);
      }
      ...
      try {
        const deleteLike = await db.deleteDocument(
          "db",
          "likes",
          doIAlreadyLikeThem.documents[0].$id,
        );
  ...
  async (req, res) => {
    ...
    if (event.type === "checkout.session.completed") {
      ...
      try {
        ...
        const mySubscription = await db.listDocuments("db", "subscriptions", [
          Query.equal("username", [registeredUser.discordUsername]),
          Query.limit(1),
          Query.orderDesc("$createdAt"),
        ]);
        ...
        if (item === "0") {
          ...
          receipt =
            "You successfully subscribed to Preference for 1 week. Enjoy!";
        } else if (item === "1") {
          // 1 month (approximate) = 30 days
          newTimestamp += 30 * 24 * 60 * 60 * 1000;
          receipt =
            "You successfully subscribed to Preference for 1 month. Enjoy!";
        } else if (item === "2") {
          // 3 months (approximate) = 3 * 30 days
          newTimestamp += 3 * 30 * 24 * 60 * 60 * 1000;
          receipt =
            "You successfully subscribed to Preference for 3 months. Enjoy!";
        } else if (item === "3") {
          // 6 months (approximate) = 6 * 30 days
          newTimestamp += 6 * 30 * 24 * 60 * 60 * 1000;
          receipt =
            "You successfully subscribed to Preference for 6 months. Enjoy!";
        } else if (item === "4") {
          // 1 year (approximate) = 365 days
          newTimestamp += 365 * 24 * 60 * 60 * 1000;
          receipt =
            "You successfully subscribed to Preference for 1 year. Enjoy!";
        }

        const createSubscription = await db.createDocument(
          "db",
          "subscriptions",
          ID.unique(),
          {
            username: registeredUser.discordUsername,
